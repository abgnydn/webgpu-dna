...
| 5 | E13c | ✗ fail → constant split | **Real-harness re-validation after the radius bump.** First attempt with shared SSB_R_DAMAGE_NM=1.0 exploded **SSB_dir from 24 → 388** (direct scoring shares the constant). Split into separate `SSB_R_DAMAGE_NM` (direct, 0.29) + `SSB_R_DAMAGE_INDIRECT_NM` (indirect, 1.0). E13c re-run after the split: SSB_dir = 24 ✓ (no regression), SSB_ind = 0 still — confirms the deeper issue is **late-time scoring** (t=1μs sees almost no OHs near DNA because IRT has consumed them by then), not radius. Real fix is to score OH-backbone encounters during the IRT timeline — pending refactor of `public/irt-worker.js`. | [E13c](./experiments/results/2026-05-11/level-5/E13c-rerun-ssb-after-fix.json) |
