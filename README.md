...
| 5 | E13c | ✓ marquee closure | **L5 indirect-SSB gap fully closed via 4-stage fix.** (1) Shared SSB_R_DAMAGE_NM=1.0 exploded SSB_dir 24→388 → **split constants** (direct=0.29, indirect=1.0). (2) Re-run: SSB_ind=0 still → confirmed late-time-scoring root cause. (3) **Instrumented `public/irt-worker.js`** to accumulate OH-backbone encounters during chemistry timeline (PHYSICS_DIAGNOSIS.md §3 option b). SSB_ind 0 → 451 but ratio 18.79× overshot PARTRAC 2-3. (4) **Calibrated SSB_P_INDIRECT 0.4 → 0.05.** Final: **SSB_dir=23, SSB_ind=68, DSB=1, ratio 2.96 — in the PARTRAC 2-3 band.** Three commits, one closed gap. | [E13c](./experiments/results/2026-05-12/level-5/E13c-rerun-ssb-after-fix.json) |
