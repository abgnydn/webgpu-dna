...
> **Shipped vs reported `RECOMB_BOOST` (read this before comparing to the paper).** The **shipped demo defaults to `RECOMB_BOOST = 2.0`** — a tuning scalar with no Geant4 physical basis (the H₂O⁺ refutation) — and the chemistry rows below report that shipped config (E10j). E10r proved the knob is **not load-bearing**: setting it to its neutral `1.0` costs only **+1.4 pp RMS** and *improves* OH/eaq/H. The **paper therefore reports the parameter-free (`1.0`) yields** (G(OH) 0.91× / eaq 0.86× …, E10r) rather than lead with a fudge. So README headline chemistry = shipped 2.0 (E10j: G(OH) 0.895×); paper = parameter-free 1.0 (E10r: G(OH) 0.91×) — the two differ by ~1.4 pp and **both are disclosed, not silently divergent**. Flipping the shipped default to 1.0 is queued behind a cascade-ion + strand-break revalidation (those endpoints also depend on the knob).
...
| Ions / primary (full cascade) — **production (post-joint-fix)** | 344.6 (H₃O⁺-corrected; 325.5 H₃O⁺-only) | 509.2 (Geant4) | **0.677× (32% deficit)** [[E7b]](./experiments/results/2026-05-12/level-2/E7b-l2-post-joint-fix-cascade.json) — was 371.9 / 0.730× pre-fix [[E7]](./experiments/results/2026-05-11/level-2/E7-ions-per-primary-cascade.json); the joint fix's `RECOMB_BOOST` lowers cascade ions (two-knob tradeoff) |
...
| SSB direct / indirect / DSB @ 10 keV (post joint-fix) | 26 / 64 / 9      | indirect/direct ratio PARTRAC = 2-3         | 2.46 ratio — ⚠ **calibrated fit** (`P_indirect` tuned). Absolute yields per *local* dose: SSB 0.34–1.28× / DSB 0.82× exp (E12-local: 223×/796× was a point-source dose artifact, C≈981) [[E12-local]](./experiments/results/2026-06-03/level-5/E12-local-dose-yield.json) [[E13c]](./experiments/results/2026-05-13/level-5/E13c-rerun-ssb-after-fix.json) |
...
5. **WGSL cascade ions/primary is 32% lower than Geant4 in production** (post-joint-fix 344.6 vs 509.2 → 0.677×; was 27% / 371.9 pre-fix). Two compounding causes: σ_exc inflation channels energy away from ionization, and the joint fix's `RECOMB_BOOST` kills source secondaries before they cascade-ionize — a two-knob tradeoff (the fix improves CSDA + chemistry but worsens the cascade count). [E7 pre-fix, E7b production]
...
9. **L5: absolute yields validate to within ~3× of experiment once normalised by local dose; the DSB/SSB *ratio* is a calibrated fit.** Two separable claims, now both measured. (a) **Absolute yields — vindicated.** The 223×/796× box-normalised over-yield was a *point-source dose artifact*: E12-local measures that 98.1% of energy deposits in the central 3 µm fibre core, so local dose is ~238 Gy (C≈981), not the 0.243 Gy box average. Re-normalised, SSB_dir 0.34× / DSB 0.82× / SSB_total 1.28× experiment (Ward 1988) — within a factor of ~3. (b) **DSB/SSB ratio — still a fit.** The indirect/direct ratio (2.46) is `reach × tuned probability` (SSB_dir=26=⌊173×0.15⌋, SSB_ind=64≈1423×0.05), with `P_indirect` tuned 0.4→0.05 to land in PARTRAC's 2–3 band — circular, and dose-normalisation doesn't touch it. So L5 predicts absolute yields to order-of-magnitude (good) but the strand-coincidence ratio is calibrated, not predicted. [E12, E12-local, E13c]
