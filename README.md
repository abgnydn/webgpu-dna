...
| SSB direct / indirect / DSB @ 10 keV (**parameter-free — no calibrated fit**) | 82 / 190 / 34 | indirect/direct ratio PARTRAC = 2-3 | **ratio 2.32 — a prediction, in-band** ([E30]): with `P_direct`=1.0 and `P_indirect`=0.13 (Nikjoo) the ratio is predicted, not tuned, and lands in PARTRAC's 2–3 band. Measured on a **fresh v0.7.0 GPU dump** (Phase A+B) + Node IRT at N=4096, and it is **stable across shader versions** — 2.32 (v0.7.0) vs 2.39 on the v0.6.0 dump — whereas the old *tuned* ratio drifted 2.46→3.26 across that same bump, evidence the physical values (not a fit) are what make it robust. Absolute counts rose vs the old tuned 32/81/17 because the probabilities are physical now. ⚠ **Treat SSB/DSB as methodology, not validated absolute physics**: the scoring layer's probabilities are now physical (`SSB_P_DIRECT`=1.0, `SSB_P_INDIRECT`=0.13 Nikjoo, [TUNABLES.md](./TUNABLES.md)), so the ratio is a **prediction** (2.32, in-band, stable across shader versions); it is also **robust to the target geometry**: a 4× fibre-spacing sweep (75→300 nm) holds the ratio at 2.24–2.53, all in-band, while absolute counts scale ~4× [[E27]](./experiments/results/2026-06-09/level-5/E27-ssb-geometry-sensitivity.json). Absolute yields per *local* dose: SSB 0.34–1.28× / DSB 0.82× exp (C=991 exact, [E12-local-exact]) [[E25]](./experiments/results/2026-06-08/level-4/E25-tertiary-cascade-CLEAN-WIN.json) [[E12-local-exact]](./experiments/results/2026-06-08/level-5/E12-local-exact.json) |
