...
| SSB direct / indirect / DSB @ 10 keV (**parameter-free — no calibrated fit**) | 27 / 192 / 33 | indirect/direct ratio PARTRAC = 2-3 | **ratio 7.1 — parameter-free, ABOVE the band (honest-negative)** ([E32]): direct SSB uses the physically-faithful **accumulated-volume energy threshold** (Nikjoo/Charlton — sum each event's deposit per sugar site, threshold once; `E_low`=5/`E_high`=37.5 eV; shaders emit per-event energy via `rad_e`). ⚠ Testing whether accumulation would rescue the ratio **refuted the idea**: near-backbone sugars are almost all hit by a *single* ionisation (0.29 nm reach too small for overlap), so accumulated (7.1) ≈ per-event (6.5, E31). The gap to PARTRAC is the **threshold itself** (mean deposit ~15 eV → only ~1/3 of near-backbone ionisations break), not the aggregation. Three parameter-free points bracket the physics: threshold-free `P=1` → 2.32 in-band [E30]; per-event ramp → 6.5 [E31]; accumulated ramp → 7.1 [E32]. None tuned. Fresh v0.7.0 GPU dump + Node IRT, N=4096. ⚠ **Treat SSB/DSB as methodology, not validated absolute physics**: the scoring layer's probabilities are now physical (direct = energy-threshold ramp `E_low`=5/`E_high`=37.5 eV, `SSB_P_INDIRECT`=0.13 Nikjoo, [TUNABLES.md](./TUNABLES.md)), so the ratio is a **prediction** (7.1, above band — see left); it is also **robust to the target geometry**: a 4× fibre-spacing sweep (75→300 nm) holds the ratio at 2.24–2.53, all in-band, while absolute counts scale ~4× [[E27]](./experiments/results/2026-06-09/level-5/E27-ssb-geometry-sensitivity.json). Absolute yields per *local* dose: SSB 0.34–1.28× / DSB 0.82× exp (C=991 exact, [E12-local-exact]) [[E25]](./experiments/results/2026-06-08/level-4/E25-tertiary-cascade-CLEAN-WIN.json) [[E12-local-exact]](./experiments/results/2026-06-08/level-5/E12-local-exact.json) |
...
- **DNA damage is methodology, not validated absolute physics.** The 21×21 fibre
  ...
  **parameter-free** — `P_indirect` (0.13, Nikjoo) and the direct energy-threshold
  ...
  and it lands **above** PARTRAC's 2–3 band (7.1, honest-negative; the threshold-free
  `P=1` limit is 2.32, in-band). The ratio is robust to grid spacing (E27). Treat
