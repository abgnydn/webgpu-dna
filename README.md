...
**Citing this work:** see [`CITATION.cff`](./CITATION.cff). The current release is `v0.8.0` — K-shell Auger cascade fix (ions 0.981×) + the explicit OH+deoxyribose damage channel shipped as default (SSB ratio 2.281, in band) ([GitHub Release](https://github.com/abgnydn/webgpu-dna/releases/tag/v0.8.0)); the track-structure physics remains the v0.7.0 real-Born, parameter-free build. Cite the Zenodo **concept** DOI [10.5281/zenodo.20506339](https://doi.org/10.5281/zenodo.20506339), which always resolves to the latest version; per-version DOIs (e.g. the v0.6.0 version DOI 10.5281/zenodo.20606566) are listed on the Zenodo record.
...
Every artifact was produced on one machine (`Ahmets-MBP.localdomain`, Apple M2 Pro); T1/T2 are what someone *else* could reproduce today. ⚠ A separate axis the tier does **not** capture: whether a row is a *calibrated fit* or a genuine *prediction*. The L5 SSB ratio **was** a calibrated fit (`P_indirect` tuned to PARTRAC's band); as of 2026-07 the scoring layer is **parameter-free**, so the ratio is a **prediction** (see §Numbers). Any row that is still a fit is flagged in-row.

**GPU coverage in CI:** the gating `ci.yml` (typecheck + lint + vitest) is GPU-free, so it never executed the WGSL physics — the long-standing gap behind these tiers. A **`webgpu-smoke`** job addresses it by running the *real* shipped shaders on a **software** WebGPU adapter (Mesa lavapipe / SwiftShader in headless Chromium): it compiles the production primary/secondary/chemistry bundles (assembled exactly as `src/shaders/loader.ts`) and runs a compute dispatch, so a WGSL regression vitest cannot see fails CI. The workflow is **active** at [`.github/workflows/webgpu-smoke.yml`](./.github/workflows/webgpu-smoke.yml) — it runs on every PR and on `push` to `main`. It is a **smoke test, not a physics re-validation** (a software adapter at small N is too noisy for the marquee ratios), and ships **non-blocking** (`continue-on-error`) until its first green run on a real runner, after which removing that line makes it gate. See `experiments/tools/webgpu-smoke.mjs` and [`ci/README.md`](./ci/README.md).
...
| CSDA @ 100 eV (vs Geant4) — **v0.7.0 Born excitation** | 25.1 nm          | 26.21 nm                                    | **0.956×** (was 0.782× @ scaled-Emf; real Born XS closes the sub-keV deficit, E29) [[E29]](./experiments/results/2026-06-09/level-2/E29-physics-list-audit-born-excitation.json) |
| CSDA @ 300 eV — **v0.7.0 Born**             | 35.4 nm          | 35.91 nm                                    | **0.986×** (was 0.852×, E29) [[E29]](./experiments/results/2026-06-09/level-2/E29-physics-list-audit-born-excitation.json) |
| CSDA @ 500 eV — **v0.7.0 Born**             | 47.8 nm          | 48.07 nm                                    | **0.994×** (was 0.894×, E29) [[E29]](./experiments/results/2026-06-09/level-2/E29-physics-list-audit-born-excitation.json) |
| CSDA @ 1 keV — **v0.7.0 Born**             | 89.2 nm          | 90.32 nm                                    | **0.987×** (was 0.933×, E29). 3/5/20 keV: 1.002/1.005/0.993× — **all 8 energies now 0.956–1.005×** [[E29]](./experiments/results/2026-06-09/level-2/E29-physics-list-audit-born-excitation.json) |
| Energy conservation                          | 100.0 %          | 99.99 %                                     | 1.000× [[E5]](./experiments/results/2026-05-11/level-2/E5-csda-vs-g4-ntuple.json) |
| Ions / primary (full cascade) — **production (v0.7.0 + K-shell Auger)** | 499.3 | 509.2 (Geant4) | **0.981×** [[E35]](./experiments/results/2026-08-03/level-2/E35-auger-cascade.json) — emitting the missing oxygen K-shell Auger electron (~503 eV, `G4UAtomicDeexcitation`) lifted the cascade from **0.942×** (Born-only [[E29]](./experiments/results/2026-06-09/level-2/E29-physics-list-audit-born-excitation.json), full tertiary cascade [[E25]](./experiments/results/2026-06-08/level-4/E25-tertiary-cascade-CLEAN-WIN.json), recovered from 0.766×) — the Auger electrons ionise downstream. Same metric (H3O⁺+H2-marker) reproduces the 479.6 Born-only value exactly. Primary-track ionisations match to 0.1% (195.4 vs Geant4 195.6, [[E20]](./experiments/results/2026-06-08/level-2/E20-ion-split.json)) — a statistical match of the per-event means, not bit-identical |
...
| G(OH) @ 1 μs vs chem6 — **production (v0.6.0 tertiary cascade)** | 1.608            | 1.710                                       | **0.94×** [[E25]](./experiments/results/2026-06-08/level-4/E25-tertiary-cascade-CLEAN-WIN.json) (was 0.914× pre-cascade; v0.7.0 Born keeps 5-species RMS 7.0%, [E29]) |
...
| G(e⁻aq) @ 1 μs vs chem6 — **production (v0.6.0 tertiary cascade)** | 1.498            | 1.694                                       | **0.887×** [[E25]](./experiments/results/2026-06-08/level-4/E25-tertiary-cascade-CLEAN-WIN.json) (was 0.858× pre-cascade) |
...
| G(H) @ 1 μs vs chem6 — **production (v0.6.0 tertiary cascade)** | 0.77             | 0.710                                       | **1.085×** (H now overshoots) [[E25]](./experiments/results/2026-06-08/level-4/E25-tertiary-cascade-CLEAN-WIN.json) (was 0.924× pre-cascade) |
| G(H₂) @ 1 μs vs chem6 — **production (v0.6.0 tertiary cascade)** | 0.618            | 0.622                                       | **0.993×** [[E25]](./experiments/results/2026-06-08/level-4/E25-tertiary-cascade-CLEAN-WIN.json) — the long-standing H₂ deficit (0.741×) is **closed** by the tertiary cascade |
| G(H₂O₂) @ 1 μs vs chem6 — **production (v0.6.0 tertiary cascade)** | 0.788            | 0.850                                       | **0.927×** (5-species RMS **7.6%** at v0.6.0; 7.0% under v0.7.0 Born [E29]; — vs a **single** chem6 run; the reference is point-values with no stated MC uncertainty, so read 7.0% as agreement to one realization) [[E25]](./experiments/results/2026-06-08/level-4/E25-tertiary-cascade-CLEAN-WIN.json) — the H₂O₂ deficit (0.693×) largely closed |
| Implicit W-value (E_total / N_ions) | 29.0 eV *(measured, pre-cascade)* | 21.4 eV (ICRU 31, low-LET liquid water)     | **1.36× — status=fail (+35.6%)** [[E5c]](./experiments/results/2026-05-12/level-2/E5c-w-value.json), the only committed W measurement. It predates the tracked tertiary cascade; recovering the missing ions (0.766→0.981×, [E25]/[E35]) is *expected* to pull W down toward ICRU, but a post-cascade W has **not** been separately archived — no in-band W is claimed here |
...
| SSB direct / indirect / DSB @ 10 keV (**parameter-free — no calibrated fit**) | 32 / 73 / 15 | indirect/direct ratio PARTRAC = 2-3 | **ratio 2.281 — parameter-free, IN PARTRAC's 2–3 band** (absolute counts measured on the committed dump [[E41]](./experiments/results/2026-08-08/level-5/E41-shipped-explicit-counts.json); mechanism [E39]/[E40]; shipped default as of 2026-08-05). The legacy `encounter` proxy gave **5.74** (above band, honest-negative [E33]); **⟶ [E39, 2026-08-04] that excess was an indirect-*model* artifact, fixed at the source:** making the deoxyribose an **explicit competing IRT reactant** (Geant4-DNA *molecularDNA* structure — an OH damages a sugar only when it actually *reacts* with it, not merely dies within 1 nm of it) collapses the ratio **5.74 → 1.66** at 10 keV, parameter-free (same `p_ssb`=0.13, same geometry, same dump, identical direct count — only the indirect mechanism changes). Direct measurement: only **~20 %** of near-backbone OHs actually react with the sugar; the shipped `encounter` proxy miscredited all of them, and its excess **climbs with track density** (ratio 2.54 → 3.13 → 4.27 → 5.74 over 1–10 keV) while the faithful channel stays **flat** (1.25 → 1.15 → 1.41 → 1.66). The initial on-line `±2bp` reaction search truncated the first-passage reach and undercounted to 1.66; **widening `dnaReactQuery` to ±7bp + a 2.5 nm react-radius gives 2.28 on-line at 10 keV — SQUARELY IN PARTRAC's 2–3 band, parameter-free** (cross-checked by the E40 offline replay's complete nearest-sugar search: 2.17, rising with energy 1.43→1.57→2.17 over 1–10 keV). So the faithful headline is **5.74 → 2.28 (in band)**. **⟶ [E40] realistic chromatin folding keeps it in band:** replaying the explicit competition against a folded 30-nm nucleosome fibre (vs a straight duplex on identical axes) gives `dR`(solenoid/line) = **0.83** at 10 keV — folding is a *second-order* lever (0.83–1.16 over 1–10 keV; the replay reproduces E37's survival `dR`=0.716 exactly as a pipeline check). So the out-of-band 5.74 was an indirect-*model* over-counting artifact, **not** geometry [E38] and **not** the direct threshold — the faithful reaction model lands in band at the mechanism level, no tuning. **Shipped as the default** (`SSB_EXPLICIT_DNA_REACTION=true`, 2026-08-05) — the live app and both deploys now score the in-band 2.28; the encounter proxy (5.74) is kept as the `false` fallback + the documented honest-negative. [[E39]](./experiments/results/2026-08-04/level-5/E39-explicit-dna-reaction.json) [[E40]](./experiments/results/2026-08-04/level-5/E40-explicit-chromatin.json) [[E41]](./experiments/results/2026-08-08/level-5/E41-shipped-explicit-counts.json)<br>Original E33 detail: direct SSB uses the **accumulated-volume energy threshold** (Nikjoo/Charlton — sum each event's deposit per sugar site, threshold once; `E_low`=5/`E_high`=37.5 eV), scored at the **true energy-deposit site** (a `rad_dep` buffer carrying the ionisation position `px`), **not** the ~2 nm mother-displaced radical position that `rad_buf` carries for the chemistry. Correcting that localization raised the in-reach ionisation count 93→112 and direct SSB **27→35** on the same dump, dropping the ratio **7.1→5.74** — toward but still above PARTRAC's band, so the **energy threshold** stays the dominant gap (mean deposit ~15 eV → only ~1/3 of near-backbone ionisations break), not localization. Bracket, all recomputed at the deposit site: threshold-free `P=1` → **2.34 in-band** [E33]; energy-threshold ramps → 5.7–6.8, above. None tuned. Fresh v0.7.0 GPU dump + Node IRT, N=4096. ⚠ **Treat SSB/DSB as methodology, not validated absolute physics**: the scoring layer's probabilities are now physical (direct = energy-threshold ramp `E_low`=5/`E_high`=37.5 eV, `SSB_P_INDIRECT`=0.13 Nikjoo, [TUNABLES.md](./TUNABLES.md)), so the ratio is a **prediction** (2.281 with the shipped explicit channel, **in band**; the legacy encounter proxy gave 5.74 above band — see left); it is also **robust to the target geometry**: a 4× fibre-spacing sweep (75→300 nm) holds the ratio at 2.24–2.53, all in-band, while absolute counts scale ~4× [[E27]](./experiments/results/2026-06-09/level-5/E27-ssb-geometry-sensitivity.json). Absolute yields per *local* dose: SSB 0.34–1.28× / DSB 0.82× exp (C=991 exact, [E12-local-exact]) [[E25]](./experiments/results/2026-06-08/level-4/E25-tertiary-cascade-CLEAN-WIN.json) [[E12-local-exact]](./experiments/results/2026-06-08/level-5/E12-local-exact.json) |
...
| End-to-end pre-DNA pipeline vs Geant4 ST     | 194.8 s          | 289.1 s                                     | **1.48× — the honest like-for-like number** (both whole-pipeline; IRT chem on CPU dominates) [[E15b]](./experiments/results/2026-05-11/level-6/E15b-vs-geant4-single-thread.json) |
...
9. **L5: absolute yields validate to within ~3× of experiment once normalised by local dose; the DSB/SSB *ratio* is now a parameter-free prediction (was a calibrated fit).** Two separable claims, both measured. (a) **Absolute yields — vindicated.** The 223×/796× box-normalised over-yield is a *point-source dose artifact*: 99.1% of energy deposits in the central 3 µm fibre core (exact voxel dose, C=991, local dose 241 Gy [E12-local-exact]), not the 0.243 Gy box average. Re-normalised, SSB_dir 0.34× / DSB 0.82× / SSB_total 1.28× experiment (Ward 1988) — within a factor of ~3. (b) **indirect/direct ratio — now a parameter-free prediction, IN BAND (2026-08).** The tuned `P_indirect` was removed: indirect uses the Nikjoo OH+deoxyribose branching (0.13), direct a Nikjoo/Charlton energy-threshold ramp. The *shipped* indirect model makes the deoxyribose an **explicit competing IRT reactant** (Geant4-DNA molecularDNA structure) — an OH damages a sugar only when it actually reacts with it — giving indirect/direct = **2.28 at 10 keV, IN PARTRAC's 2–3 band, parameter-free** [E39/E40]. The earlier `encounter` proxy (score every OH that dies within 1 nm of a backbone) over-counted — only ~20% of near-backbone OHs actually react with the sugar — inflating the ratio to **5.74** (above band, [E33]); that was an indirect-*model* artifact, **not** the energy threshold and **not** geometry (folding is a second-order dR≈0.83–1.16 lever, E38/E40). [E39, E40, E12-local-exact]
...
- **Residual ~1.9 % cascade-ion deficit** (0.981× vs Geant4 after the K-shell
  Auger fix, E35; was 5.8 % / 0.942× Born-only; entirely the
...
- **DNA damage is methodology, not validated absolute physics.** The 21×21 fibre
  grid is a track-core stand-in, not chromatin. The scoring layer is now
  **parameter-free** — `P_indirect` (0.13, Nikjoo) and the direct energy-threshold
  ramp are physical, not tuned — so the indirect/direct ratio is a **prediction**,
  and with the shipped **explicit OH+deoxyribose reaction channel** (molecularDNA
  structure) it lands **IN** PARTRAC's 2–3 band (**2.28** at 10 keV, E39/E40); the
  earlier encounter proxy's out-of-band 5.74 was an over-counting artifact, not
  geometry (fold dR≈0.83–1.16, E40). The ratio is robust to grid spacing (E27) and
  chromatin folding keeps it in band. Treat SSB/DSB as indicative; the fullest
  validation still wants molecularDNA geometry (**E14**, deferred).
