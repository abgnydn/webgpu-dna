...
| ID | Status | Result | Artifact |
|:---|:------:|:-------|:---------|
| E12  | ⚠ calibrated, **not** validated vs experiment | SSB/DSB vs experiment-calibrated cellular yields (~35 DSB, ~1000 SSB per cell·Gy, low-LET [Ward 1988]; E12's "Friedland" per-Da numbers equal these). DSB/SSB = 0.083 (2.4–3.6× experiment's 0.023–0.035; "passes" only a deliberately loose factor-5 band, and DSB count = 2 is noisy). **Absolute per-Da yields miss experiment by 223× (SSB) / 796× (DSB)** normalised by box-average dose. Direction is consistent with track-core dose concentration, but that concentration factor was **never computed** — the absolute yields are neither validated nor quantitatively explained. Missing terminal test = **E12-local** (re-normalise by the *local* dose the fibre grid actually receives, from the voxel dose grid). | [E12](./experiments/results/2026-05-11/level-5/E12-ssb-yield-vs-friedland.json) |
...
| SSB direct / indirect / DSB @ 10 keV (post joint-fix) | 26 / 64 / 9      | indirect/direct ratio PARTRAC = 2-3         | 2.46 — ⚠ **calibrated fit** (`P_indirect` tuned to land here), not a prediction; absolute yields 223×/796× vs experiment, unexplained [[E12]](./experiments/results/2026-05-11/level-5/E12-ssb-yield-vs-friedland.json) [[E13c]](./experiments/results/2026-05-13/level-5/E13c-rerun-ssb-after-fix.json) |
...
| Phase A + B vs Geant4 11.4.1 single-thread   | 635 ms (dispatch-only) | 289.1 s (median/3) **whole-process** | 455× — ⚠ **not apple-to-apple**: G4 time includes init + DNA table-build + ntuple I/O; WGSL is dispatch-only. Pure-tracking ratio is lower (see finding #6, E15-fair pending) [[E15b]](./experiments/results/2026-05-11/level-6/E15b-vs-geant4-single-thread.json) |
| Phase A + B vs Geant4 MT-8                   | 635 ms (dispatch-only) | 178.0 s (median/3)              | 280× — ⚠ same caveat; MT-8 scales only **1.62×** vs ST → large fixed/serial fraction counted only against G4 [[E15c]](./experiments/results/2026-05-11/level-6/E15c-vs-geant4-multi-thread.json) |
...
6. **WebGPU tracking is 455× faster than Geant4 11.4.1 single-thread — but read the apple-to-apple caveats, the honest like-for-like figure is 1.48× end-to-end.** Two corrections to the earlier framing: (a) **The 455×/280× are not apple-to-apple.** The Geant4 wall-clock is the *whole process* — init + DNA physics-table construction + event loop + per-event ntuple I/O — while the WGSL 635 ms is *dispatch-only* (no device init, no shader compile, no dump write). That Geant4 MT-8 scales only **1.62×** (not 8×) shows a large fixed/serial fraction counted only against Geant4; a 2-point Amdahl read puts ~160 s of the 289 s in serial overhead, so the pure-tracking speedup is materially lower (rough estimate ~200×, to be pinned by E15-fair). (b) **Kernel fusion contributes ~2× to the pipeline, NOT 40×.** The fused phase (Phase A) is 14.4 ms = 2% of the 635 ms; Phase B is an *un-fused* 2000-dispatch wavefront (620 ms). The earlier "455× = 10× GPU × 40× fusion (multiplicative)" claim was wrong — you cannot multiply a Phase-A-only 40× through a 98%-un-fused pipeline. Fused-vs-naive on the *same GPU* is 1324 ms → 635 ms = 2.08× (E16's 40× is Phase-A-only). [E15b, E15c, E16]
...
9. **L5 is a calibrated damage model, not a validated prediction — the honest correction.** The indirect/direct ratio (2.46) is `reach × tuned probability` (SSB_dir=26=⌊173×0.15⌋, SSB_ind=64≈1423×0.05), with `P_indirect` tuned 0.4→0.05 *specifically* to land in PARTRAC's 2–3 band — so it is circular, and PARTRAC is itself a simulation. Against experiment-calibrated cellular yields (~35 DSB / ~1000 SSB per cell·Gy, Ward 1988), absolute per-Da yields miss by **223× (SSB) / 796× (DSB)** on box-average dose; the track-core concentration that would explain this was never computed. L5 demonstrates the clustering kernel discriminates strand-break coincidences PARTRAC-like — nothing more. The missing terminal test is **E12-local** (yields per *local* dose). [E12, E13, E13b, E13c]
