...
- **T3 — author-local.** Needs something not in the repo or the releases: the 6.76 GB Geant4 `dna.root` ntuple (per-`trackID` splits), a local Geant4 build's wall-clock, or a live run on specific GPU/browser hardware. Not reproducible by a third party without rebuilding the oracle. *(Performance rows; the multi-energy GPU sweeps; ntuple-split rows.)*
