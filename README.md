...
| SSB direct / indirect / DSB @ 10 keV (**parameter-free — no calibrated fit**) | 30 / 195 / 29 | indirect/direct ratio PARTRAC = 2-3 | **ratio 6.5 — a parameter-free prediction, ABOVE the band (honest-negative)** ([E31]): direct SSB now uses the **energy-threshold ramp** (Nikjoo/Charlton, break prob ∝ per-event deposit, `E_low`=5, `E_high`=37.5 eV) instead of a flat probability — the shaders emit per-event energy. Mean in-reach deposit ≈15 eV → only ~1/3 of near-backbone ionisations break, so the **per-event** threshold under-counts direct SSB vs the **accumulated-volume** energy PARTRAC integrates. The cruder threshold-free limit (`P=1`) predicts 2.32, in-band [E30]; the 2.32↔6.5 span measures direct-SSB sensitivity to sub-threshold accounting. Both are parameter-free; neither is tuned. Fresh v0.7.0 GPU dump + Node IRT, N=4096. ⚠ **Treat SSB/DSB as methodology, not validated absolute physics**: the scoring layer's probabilities are now physical (direct = energy-threshold ramp `E_low`=5/`E_high`=37.5 eV, `SSB_P_INDIRECT`=0.13 Nikjoo, [TUNABLES.md](./TUNABLES.md)), so the ratio is a **prediction** (6.5, above band — see left); it is also **robust to the target geometry**: a 4× fibre-spacing sweep (75→300 nm) holds the ratio at 2.24–2.53, all in-band, while absolute counts scale ~4× [[E27]](./experiments/results/2026-06-09/level-5/E27-ssb-geometry-sensitivity.json). Absolute yields per *local* dose: SSB 0.34–1.28× / DSB 0.82× exp (C=991 exact, [E12-local-exact]) [[E25]](./experiments/results/2026-06-08/level-4/E25-tertiary-cascade-CLEAN-WIN.json) [[E12-local-exact]](./experiments/results/2026-06-08/level-5/E12-local-exact.json) |
