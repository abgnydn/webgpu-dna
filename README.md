...
| Phase A + B vs Geant4 single-thread   | 635 ms     | 289.1 s (Geant4 11.4.1 ST median, 3 trials) | **455× speedup**³ | [[E15b]](./experiments/results/2026-05-11/level-6/E15b-vs-geant4-single-thread.json) |
| Phase A + B vs Geant4 multi-thread MT-8 | 635 ms   | 178.0 s (Geant4 MT-8 median, 3 trials) | **280× speedup**³ (production-realistic baseline) | [[E15c]](./experiments/results/2026-05-11/level-6/E15c-vs-geant4-multi-thread.json) |
| End-to-end pre-DNA vs Geant4 ST       | 194.6 s    | 289.1 s                         | 1.48× speedup⁴     | [[E15b]](./experiments/results/2026-05-11/level-6/E15b-vs-geant4-single-thread.json) |
...
| 2 | E8  | partial pass (7/8 bins) | **Secondary KE spectrum at creation** vs Geant4 ntuple (parentID=1, KE≥7.4 eV). Sec/primary: WGSL **143.4 vs G4 144.9** (1.0% match). 7 of 8 significant log-bins from 6 eV to 800 eV agree within **0.1-3.1%** — the Born differential CDF sampling matches Geant4 essentially exactly across the bulk. Only the 438-806 eV tail bin shows a 43% deficit (WGSL 0.69% vs G4 1.23%, ~2.5σ). | [E8](./experiments/results/2026-05-11/level-2/E8-secondary-ke-spectrum.json) |
| 3 | E9  | ✗ fail (honest negative) | **Pre-chemistry G(species) @ 0.1 ps vs Geant4 11.4.1 chem6** at matched 10 keV — closes "is the gap pre-chemistry or IRT?". Result: OH 0.87× / eaq 0.90× / H 0.88× / **H₂ 0.51× / H₂O₂ 0.58×** at 0.1 ps. **Localizes the E10c 1 μs deficit to pre-chemistry, not IRT reaction rates** (which match chem6 line-for-line). See [PHYSICS_DIAGNOSIS.md](./PHYSICS_DIAGNOSIS.md) §1 for the propagation table + concrete fix candidates. | [E9](./experiments/results/2026-05-11/level-3/E9-prechem-vs-chem6.json) |
...
| 4 | E10d | partial pass (24/25 cells) | **chem6 matched-LET sweep across all 5 V-shape energies (1/3/5/10/20 keV).** **24 of 25 species×energy cells in 30% band; only G(H₂O₂) at 20 keV fails at 0.66×.** Systematic patterns clear: G(OH) ~0.88× uniformly, G(eaq) ~0.83× uniformly, G(H) ~1.00× (perfect match across all LET), G(H₂) and G(H₂O₂) deficits grow with energy. **chem6 INDEPENDENTLY reproduces the V-shape**: chem6 G(eaq) drops 1.36 (1 keV) → 1.26 (3 keV) → 1.41 (5 keV) — confirms E10/E10b's V-shape is real LET physics, not an IRT-side artifact. | [E10d](./experiments/results/2026-05-11/level-4/E10d-vs-chem6-multi-energy.json) |
| 4 | E11 | ✗ fail (honest negative) | **GPU chem backend vs IRT worker on the same rad bin (matched input).** GPU primary species match IRT within 5% at t ≤ 100 ps; diverge upward at long times (1.94× G(OH) at 100 ns, **2.33× at 1 μs**) — GPU under-counts long-time recombination so more OH/eaq survive. Molecular products consistently low: G(H₂) 0.18-0.31×, G(H₂O₂) 0.29-1.08× of IRT. **GPU runs in 14.2 s vs IRT's 194 s (13.6× faster, but inaccurate at long times)** — quantifies why `DEFAULT_CHEM_BACKEND='worker'`. | [E11](./experiments/results/2026-05-11/level-4/E11-gpu-chem-vs-irt.json) |
...
| 6 | E15c | ✓ | **Production-realistic comparison: WGSL vs Geant4 MT-8** (3 trials, M2 Pro 8 threads). Geant4 MT-8 median 178.0 s → **280× speedup vs WGSL Phase A+B**. Geant4's MT scaling is only 1.6× over single-thread (well below the theoretical 8×) because per-event task scheduling + memory contention limit parallel efficiency on this workload. The 280× number is the honest "what production Geant4 users see" speedup; 455× is the within-protocol L6 number. | [E15c](./experiments/results/2026-05-11/level-6/E15c-vs-geant4-multi-thread.json) |
...
| 5 | E13c | ✓→fix applied | **Three-stage closure of the indirect-SSB gap.** (1) First run with shared SSB_R_DAMAGE_NM=1.0 exploded SSB_dir 24 → 388 — split constants. (2) Re-run after split: SSB_dir = 24 ✓, SSB_ind still 0 → confirmed late-time scoring root cause. (3) Instrumented `public/irt-worker.js` to accumulate OH-backbone encounters during the chemistry timeline (option b in PHYSICS_DIAGNOSIS.md §3). Final E13c result: **SSB_ind 0 → 451** (lifted; first time non-zero). New honest finding: ratio overshoots PARTRAC's 2-3 at 18.79× — `SSB_P_INDIRECT = 0.4` is too generous in the IRT-side accumulator regime. Calibration tuning is the next step, not architecture. | [E13c](./experiments/results/2026-05-12/level-5/E13c-rerun-ssb-after-fix.json) |
