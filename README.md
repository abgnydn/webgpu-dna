...
9. **L5: absolute yields validate to within ~3× of experiment once normalised by local dose; the DSB/SSB *ratio* is now a parameter-free prediction (was a calibrated fit).** Two separable claims, both measured. (a) **Absolute yields — vindicated.** The 223×/796× box-normalised over-yield is a *point-source dose artifact*: 99.1% of energy deposits in the central 3 µm fibre core (exact voxel dose, C=991, local dose 241 Gy [E12-local-exact]), not the 0.243 Gy box average. Re-normalised, SSB_dir 0.34× / DSB 0.82× / SSB_total 1.28× experiment (Ward 1988) — within a factor of ~3. (b) **DSB/SSB ratio — now a parameter-free prediction (2026-07).** The tuned `P_indirect` was removed: indirect uses the Nikjoo OH+deoxyribose branching (0.13), direct a Nikjoo/Charlton energy-threshold ramp. The ratio is *predicted*, not tuned, and lands **above** PARTRAC's 2–3 band under any energy threshold — accumulated-volume **5.74** [E33], scored at the true energy-deposit site (correcting a ~2 nm localization error that read the displaced radical position — this raised direct SSB 27→35 and dropped the ratio 7.1→5.74); the threshold-free `P=1` limit gives **2.34, in-band** [E33]. The gap to the band is the **energy threshold**, not localization or per-event-vs-accumulated aggregation (near-backbone sugars are single-hit). [E12-local-exact, E13d, E33]
...
- **DNA damage is methodology, not validated absolute physics.** The 21×21 fibre
  ...
  **parameter-free** — `P_indirect` (0.13, Nikjoo) and the direct energy-threshold
  ...
  and it lands **above** PARTRAC's 2–3 band (5.74, honest-negative; the threshold-free
  `P=1` limit is 2.34, in-band). The ratio is robust to grid spacing (E27). Treat
  SSB/DSB as indicative; the real validation needs molecularDNA geometry
  (**E14**, deferred — needs the example built).
