...
- **L2**: CSDA 0.988× @ 10 keV (3.59σ) [E5]; E5b shows the deficit is energy-dependent (0.587× @ 100 eV → 0.992× @ 20 keV); cascade ions **23% low in production** (parameter-free 389.9/0.766× [E7d]; recovered from 0.677× @ RECOMB=2.0 — the RECOMB→1.0 flip stopped destroying autoionisation ions; residual 23% is σ_exc=0.5); secondary KE spectrum 7/8 bins within 0.1-3.1% [E8].
- **L3**: G(species) @ 0.1 ps pre-chem vs chem6 — OH/eaq/H all ~12% low, H₂/H₂O₂ ~50% low [E9].
- **L4**: G-values vs chem6 @ 10 keV — **parameter-free production** (`RECOMB_BOOST=1.0` as of 2026-06-08, E10r: G(OH) 0.914×, eaq 0.858×, H 0.928×, H₂ 0.741×, H₂O₂ 0.693×, RMS 19.7%). README, paper, and shipped demo now agree (the old shipped-vs-reported split is gone — the RECOMB→1.0 flip passed all three gates E7d/E10r/E13d). (Pre-flip 2.0 was E10j: OH 0.895×/eaq 0.815× with an H overshoot.) G(eaq) V-shape 1→3 keV @ 126σ [E10b]. E10e refuted cross-event recomb (3.5%); E10f confirmed per-primary partitioning is 96% of the 1 μs gap.
