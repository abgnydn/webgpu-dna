...
- **L2**: CSDA 0.988× @ 10 keV (3.59σ) [E5]; E5b shows the deficit is energy-dependent (0.587× @ 100 eV → 0.992× @ 20 keV); cascade ions 32% low in production (post-joint-fix 344.6/0.677× [E7b]; was 27%/371.9 pre-fix [E7] — RECOMB_BOOST lowers the cascade count); secondary KE spectrum 7/8 bins within 0.1-3.1% [E8].
- **L3**: G(species) @ 0.1 ps pre-chem vs chem6 — OH/eaq/H all ~12% low, H₂/H₂O₂ ~50% low [E9].
- **L4**: G-values vs chem6 @ 10 keV. **Shipped vs reported config:** shipped demo = `RECOMB_BOOST=2.0` (E10j: G(OH) 0.895×, eaq 0.815×) — README headline reports this; paper reports parameter-free `1.0` (E10r: G(OH) 0.91×, eaq 0.86×) because the knob has no Geant4 basis and is not load-bearing (+1.4 pp). Both disclosed, not silently divergent. Flipping shipped→1.0 is queued behind cascade/SSB revalidation. (Pre-fix baseline was E10c OH 0.91×/eaq 0.83×.) G(eaq) V-shape 1→3 keV @ 126σ [E10b]. E10e refuted cross-event recomb (3.5%); E10f confirmed per-primary partitioning is 96% of the 1 μs gap; E10i joint fix (σ_exc=0.5, B=2.0) lifts RMS dev 30% → 19%.
- **L5**: two separable claims. **Absolute yields — vindicated** (E12-local, 2026-06-03): the 223×/796× box-normalised over-yield is a **point-source dose artifact** (98.1% of energy in the central 3 µm core, C≈981, local dose ≈238 Gy); per local dose, SSB_dir 0.34× / DSB 0.82× / SSB_total 1.28× experiment (Ward 1988) — within ~3×. **DSB/SSB ratio — calibrated fit** (2.46; `P_indirect` tuned 0.4→0.05 to PARTRAC's band; dose-invariant). Open follow-up: E12-bulk (spread tracks, `start_half`=box). [E12, E12-local, E13c]
