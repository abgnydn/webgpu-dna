...
8-thread build) on an Apple~M2~Pro, but we are explicit about scope:
...
negligible ${\sim}\SI{2}{s}$ (so event-loop-only the figure is $452\times$),
...
\SI{635}{ms} is dispatch-only, but we measured the difference and it is small
...
genuine event-loop, and event-loop-only the speedup is $452\times$,
statistically identical to $455\times$. The one material asymmetry is
...
chemistry RMS of $7.6\,\%$ vs.\ \texttt{chem6}, at 455$\times$/280$\times$ the
