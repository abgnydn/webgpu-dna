...
wall-clock---we report all of these and are explicit about which is which. We also report the project's honest limits: a 23\,\%
cascade-ionisation deficit traceable to an intentional excitation
cross-section choice (the remaining \texttt{SIGMA\_EXC\_SCALE} divergence; the
formerly-compounding \texttt{RECOMB\_BOOST} fudge has been removed, recovering
the count from 32\,\%), and the finding that the \emph{inter-track} chemistry
...
validated end-to-end: cascade ions \emph{recover} from $0.677$ to $0.766\times$
(E7d, it had been destroying autoionisation ions), the chemistry shifts by only
$1.4$~pp RMS while \emph{improving} OH/e$^-_{\mathrm{aq}}$/H (E10r), and the
SSB indirect/direct ratio \emph{holds} in PARTRAC's 2--3 band at $2.32$ with no
recalibration (E13d). The only remaining non-unity scalar,
...
(median $0.941$, E6). In production (parameter-free) the full-cascade
ionisation count is \num{389.9} per primary vs.\ Geant4's \num{509.2}---a
23\,\% deficit (E7d), with an implicit $W$-value of ${\sim}\SI{25.6}{eV}$ vs.\
ICRU~31's \SI{21.4}{eV} (\cite{icru31}, derived from the recovered count).
Source archaeology found the deficit had two causes: the intentional excitation
inflation channels energy away from ionisation (the residual 23\,\%), and
\texttt{RECOMB\_BOOST}$=2.0$ had been \emph{destroying} ${\sim}$a third of the
autoionisation ions via boosted geminate recombination. Setting it to the
physical $1.0$ recovered them (E7d, $0.677\to0.766\times$) while keeping the
chemistry parameter-free (E10r)---dissolving the former two-knob tradeoff rather
than hiding it. The
...
Ions / primary (production) & 389.9 & 509.2 & 0.766 (E7d) \\
...
parameter-free production ($\mathrm{SSB_{dir}}=28$, $\mathrm{SSB_{ind}}=65$),
the indirect/direct ratio is $2.32$, within PARTRAC's 2--3 band
