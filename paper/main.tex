...
per-event ntuple I/O --- Geant4 writes ${\sim}\SI{6.8}{GB}$ to disk over the
full run (the row-wise merge is the likely cause of the poor $1.62\times$
MT-8 scaling), which the WGSL dispatch does not; a no-ntuple Geant4 build
would lower its number somewhat, so $452\times$ is a mild over-estimate. (We
