...
\begin{figure}[t]
\placeholderfig{Phase A/B/C pipeline + buffer map}{Generate from
ARCHITECTURE.md: Phase~A one fused dispatch (1 thread/primary) writing the
\texttt{rad\_buf} and secondary buffer; Phase~B wavefront over secondaries;
Phase~C IRT worker; atomic voxel grid for dose/radical deposition.}
...
\begin{figure}[t]
\placeholderfig{CSDA range ratio vs.\ energy, pre/post joint-fix}{Plot from
E5b (pre) and E5d (post) artifacts: WebGPU/Geant4 CSDA ratio at 100, 300, 500,
1000, 3000, 5000, 10000, 20000 eV; two curves (pre-fix $0.587\!\to\!0.992$,
post-fix $0.736\!\to\!0.996$); the post curve sits above the pre curve, gap
largest at low E.}
...
\begin{figure}[t]
\placeholderfig{$G(t)$ for five species, WebGPU vs.\ chem6}{Plot from E10c/E10j
artifacts: $G$(OH, $\mathrm{e^-_{aq}}$, H, $\mathrm{H_2}$, $\mathrm{H_2O_2}$)
vs.\ time on a log axis $0.1\,\mathrm{ps}\to1\,\mu\mathrm{s}$; solid = WebGPU,
dashed = chem6.}
\caption{Radiolysis $G$-value time evolution against Geant4 \texttt{chem6}.}
\label{fig:gvalues}
\end{figure}
...
\begin{figure}[t]
\placeholderfig{Speedup bars + throughput vs.\ $N$}{From E15/E15b/E15c/E15d/E16:
(a) bar chart 455$\times$ (ST), 280$\times$ (MT-8), 40$\times$ (fusion),
1.48$\times$ (end-to-end); (b) throughput vs.\ $N$ across 8 energies.}
...
\begin{figure}[t]
\placeholderfig{Bridge convergence ratio vs.\ $\Delta t$ at several separations}{From
E10Q: SBS/analytic reaction-probability ratio vs.\ $\Delta t$ (4, 1, 0.25,
0.05 ns) for $r_0=$ 1, 10, 30, 100 nm; the four curves overlap, showing the
$\Delta t$ requirement is set by $\sigma$, not separation.}
