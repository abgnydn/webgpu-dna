...
it (ions $0.766\to0.942\times$) and, importantly, \emph{also} closes most of the
residual \SI{1}{\micro s} chemistry gap against \texttt{chem6} (five-species RMS
$19.7\to7.0\,\%$; the $\mathrm{H_2}$/$\mathrm{H_2O_2}$ deficits close)---revising
...
ionisation, Born excitation, elastic and vibrational scattering---in a
...
\cite{karamitros2011,karamitros2014}. We validate against a single, consistent
Geant4 reference: a source audit (E29) confirms that \emph{both} oracles we
compare to---\texttt{dnaphysics} (track structure) and \texttt{chem6}
(chemistry)---register the same constructor, \texttt{G4EmDNAPhysics\_option2},
so there is no physics-list seam between the cascade and chemistry comparisons.
For \texttt{option2}, \texttt{G4EmDNABuilder} sets the Emfietzoglou energy limit
to zero, so the constructor uses \texttt{G4DNABornExcitationModel} (and Born
ionisation) across the whole electron range. Our track-structure physics
therefore matches \texttt{option2} model-for-model, and is \emph{parameter-free}:
two scalars that were once in the codebase have both been removed. The first,
\texttt{RECOMB\_BOOST}, had no physical basis; we measured it inconsequential
(E10r) and set it to $1.0$ (v0.5.0). The second, \texttt{SIGMA\_EXC\_SCALE},
flat-scaled an Emfietzoglou excitation table to approximate Born; because the
Emfietzoglou/Born ratio is energy-dependent ($\sim2.5\times$ at \si{keV} but
$\sim10\times$ at ${\sim}\SI{10}{eV}$) a single scalar left low-energy secondaries
over-excited ${\sim}4\times$. In v0.7.0 we replaced it with the genuine Born
excitation cross section (\texttt{sigma\_excitation\_e\_born}) and removed the
scalar (E29); this \textbf{closed the chronic sub-keV CSDA deficit}---the range
where track-structure codes matter most---lifting \SI{100}{eV} from
$0.78\times$ to $0.96\times$ Geant4, with all eight energies now within
\numrange{0.956}{1.005}. Separately, tracking the full \emph{tertiary} electron
cascade (v0.6.0)---previously the secondary shader absorbed tertiary electrons in
place---resolved the cascade-ion deficit (ions $0.766\to0.942\times$) and closed
the long-standing $\mathrm{H_2}$/$\mathrm{H_2O_2}$ chemistry deficits
(five-species RMS $19.7\to7.0\,\%$ vs.\ chem6) (E20--E29).
...
\midrule
Excitation & Born & Born & matched (v0.7.0, E29) \\
...
Excitation $\sigma_{\text{exc}}$ & Born (5 lev.) & 0.9970 & E2 \\
...
\SI{100}{eV} ratio rose from $0.587\times$ to $0.956\times$ (v0.7.0, real Born excitation) and all eight
...
is \num{479.6} per primary vs.\ Geant4's \num{509.2}---a 5.8\,\% deficit (E29),
...
recovers cascade ions $0.766\to0.942\times$ and, far from costing accuracy,
