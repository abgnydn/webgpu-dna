...
wall-clock---we report all of these and are explicit about which is which. We also report the project's honest limits: a 32\,\%
cascade-ionisation deficit (in production, post-joint-fix) traceable to an
intentional excitation cross-section choice compounded by the recombination
boost, and the finding that the \emph{inter-track} chemistry
...
E10r) and report all chemistry at its neutral value $1.0$, so the
$G$-values of \S\ref{sec:results} are parameter-free in that knob. We disclose
that the \emph{shipped} browser demo currently defaults to
\texttt{RECOMB\_BOOST}$=2.0$, matching the committed validation artifacts;
because the knob also feeds the cascade-ion and strand-break endpoints,
flipping the shipped default to $1.0$ is held pending their revalidation, and
the two settings differ by only $1.4$~pp RMS in the chemistry (E10r).
...
(median $0.941$, E6). In production (post-joint-fix) the full-cascade
ionisation count is \num{344.6} per primary vs.\ Geant4's \num{509.2}---a
32\,\% deficit (E7b; the pre-fix value was \num{371.9}, a 27\,\% deficit at
$263\sigma$, E7)---and the implicit $W$-value is \SI{29.02}{eV} vs.\ ICRU~31's
\SI{21.4}{eV} (\cite{icru31}, $+35.6\,\%$, E5c). Two compounding causes: the
intentional excitation inflation channels energy away from ionisation, and the
joint fix's recombination boost (which improves CSDA and chemistry) kills some
source secondaries before they cascade-ionise---a two-knob tradeoff we report
...
Ions / primary (production) & 344.6 & 509.2 & 0.677 (E7b) \\
