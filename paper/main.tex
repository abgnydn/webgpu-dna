...
\num{0.988}$\times$ Geant4. The \emph{fused track-structure kernel} is
\textbf{455$\times$} faster than Geant4 single-thread (280$\times$ vs.\ an
8-thread build) on an Apple~M2~Pro; \emph{end-to-end}, however, the speedup is
\textbf{1.48$\times$}, because the IRT radiolysis chemistry runs on CPU and
dominates the wall-clock---we report both and are explicit about which is
which. We also report the project's honest limits: a 27\,\%
...
\textbf{Caveat up front: the chemistry yields below are not parameter-free.}
They are reported with an empirical tuning scalar active
(\texttt{RECOMB\_BOOST}~$=2.0$, Table~\ref{tab:diverge}), which has no
first-principles basis and was chosen to improve agreement with
\texttt{chem6}. They should therefore be read as \emph{agreement after
tuning}, not as a parameter-free validation of the chemistry stage. Reporting
the parameter-free yields---by removing this knob and instead modelling its
intended effect physically (super-excitation autoionisation, or an
Onsager/Mozumder escape-probability treatment of geminate recombination)---is
required before the chemistry stage can be called validated, and is left to
future work (Sec.~\ref{sec:discussion}).

With that caveat, against Geant4's \texttt{chem6} at matched \SI{10}{keV} the
tuned $G$-values at \SI{1}{\micro s} (E10j) are: $G(\mathrm{OH})$
...
\SI{0.1}{ps} is 19.0\,\% (down from 30.3\,\% before tuning, E10i). One result
is, however, tuning-independent and worth highlighting:
...
\caption{Radiolysis $G$-value time evolution (WebGPU, lines) against Geant4
\texttt{chem6} endpoints at \SI{1}{\micro s} (open circles). These yields are
tuning-dependent; see the caveat in the text.}
\label{fig:gvalues}
\end{figure}
